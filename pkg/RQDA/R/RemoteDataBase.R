newMySQLProjMySQL <- function(user='rqda',password='rqdaproj',dbname='rqda',host='localhost',...)
{
    ...
    dbGetQuery(con,"create table attributes (name text, status integer, date text, dateM text, owner text,memo text, class text)")
    if (dbExistsTable(con,"caseAttr")) dbRemoveTable(con, "caseAttr")
    dbGetQuery(.rqda$qdacon,"create table caseAttr (variable text, value text, caseID integer, date text, dateM text, owner text, status integer)")
    if (dbExistsTable(con,"fileAttr")) dbRemoveTable(con, "fileAttr")
    dbGetQuery(.rqda$qdacon,"create table fileAttr (variable text, value text, fileID integer, date text, dateM text, owner text, status integer)")
    ...
    dbGetQuery(.rqda$qdacon,"create table annotation (fid integer,position integer,annotation text, owner text, date text,dateM text, status integer)")
    if (dbExistsTable(con,"image")) dbRemoveTable(con, "image")
    dbGetQuery(con,"create table image (name text, id integer, date text, dateM text, owner text,status integer)")
    if (dbExistsTable(con,"imageCoding")) dbRemoveTable(con, "imageCoding")
    dbGetQuery(con,"create table imageCoding (cid integer,iid integer,x1 integer, y1 integer, x2 integer, y2 integer, memo text, date text, dateM text, owner text,status integer)")
}

openMySQLProject <- function(user='rqda',password='rqdaproj',dbname='rqda',host='localhost',updateGUI=FALSE,...) {
